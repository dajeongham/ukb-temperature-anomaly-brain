...
cat("참가자 수:", nrow(dt), "| 변수 수:", ncol(dt), "\n")
...
dt[, p22189    := as.numeric(p22189)]      # Townsend
dt[, SBP       := as.numeric(p4080_i2_a0)]
dt[, DBP       := as.numeric(p4079_i2_a0)]
...
covar_m4 <- c(covar_m3, "followup_years","p54_i2","p54_i3")   # 기준 모델
...
results_dcnt_adj <- rbindlist(list(
  ...
  run_all_dcnt_adj(exposures_main, covar_m4, "M4b_scan_dcntadj"),   # 메인
...
cat("\n저장 완료: results_model_comparison.csv\n")
